...
| 更新日 | 2026-03-03 |
| 入力元 | REQ-003, REQ-005 |
| ステータス | レビュー修正済（REV-001 ISS-001対応） |
...
> **フェーズ1 スコープ注記**: REQ-004（NFR-SEC-05）および BSD-001/BSD-002（CON-B-01）の定義により、フェーズ1ではシングルユーザー・ローカル環境のため認証機能は不要。SCR-001（ログイン画面）はフェーズ2スコープとして保留する。

| 画面ID | 画面名 | URL/パス | 説明 | 遷移元 | 遷移先 |
|---|---|---|---|---|---|
| SCR-001 | ログイン画面 | `/login` | ユーザー認証画面 **[フェーズ2スコープ: フェーズ1では未実装]** | 未認証時すべての画面 | SCR-002 |
| SCR-002 | ダッシュボード | `/` | タスク概要・エージェント状態の一覧表示 | 直接アクセス可能（フェーズ1では認証不要） | SCR-003, SCR-004, SCR-005 |
...
> **フェーズ1**: 認証不要のため `/login` へのリダイレクトなし。アクセス時は直接ダッシュボード（SCR-002）から開始。
> **フェーズ2**: SCR-001（ログイン画面）を追加し、未認証時はログイン画面へリダイレクトする。
...
| コンポーネント名 | 説明 | 使用画面 |
|---|---|---|
| ヘッダー | グローバルナビゲーション（ロゴ・メニュー）・ユーザー情報 ※ログアウトボタンはフェーズ2スコープ | SCR-002〜008 |
...
| フォーム共通スタイル | 入力・ラベル・バリデーションエラー表示 | SCR-006, SCR-007（SCR-001はフェーズ2） |

---

## 5. 画面別ワイヤーフレーム

### 5.1 SCR-001: ログイン画面 **[フェーズ2スコープ]**

> **注意**: フェーズ1ではシングルユーザー・ローカル環境のため、このログイン画面は実装しない（REQ-004 NFR-SEC-05、BSD-001/BSD-002 CON-B-01 に基づく）。フェーズ2以降に認証機能を追加する際に実装すること。
