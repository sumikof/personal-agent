...
| 更新日 | 2026-03-03 |
| 入力元 | REQ-002, REQ-005 |
| ステータス | レビュー修正済（REV-001 ISS-001/ISS-002対応） |
...
2. **タスク管理（CRU）**: Redmine に対してタスクの作成・参照・更新を行う（BR-02: タスク削除はエージェント経由では実行しない。削除は Redmine Web UI からの手動操作のみ）
...
| BF-005 | ログイン認証 | ACT-001 | ユーザー認証・セッション開始 **[フェーズ2スコープ: フェーズ1では未実装]** |
...
- エージェントはタスク削除操作を行わない（BR-02: タスク削除は Redmine Web UI からの手動操作専用）
...
- タスクの作成対象は Redmine 上の既存プロジェクトに限る（BR-01: 新規プロジェクトの作成は不可）
- タスク削除操作はエージェントを通じて実行しない（BR-02: 削除は Redmine Web UI からの手動操作専用）
...
- タスク削除はエージェント経由で実行しない（BR-02: 削除は Redmine Web UI からの手動操作専用）
...
### 3.5 BF-005: ログイン認証 **[フェーズ2スコープ]**

> **注意**: フェーズ1ではシングルユーザー・ローカル環境のため、このフローは実装しない（REQ-004 NFR-SEC-05、BSD-001/BSD-002 CON-B-01 に基づく）。フェーズ2以降に認証機能を追加する際に実装すること。
...
| ログイン **[フェーズ2]** | フォーム送信・トークン保存 | パスワード照合・JWT 発行 | POST /api/v1/auth/login |
