...
### 2.2 ドメインモデルの役割分担（ISSUE-M03対応）

CTX-001（タスク管理コンテキスト）では、Redmine Issueを表現するモデルが2種類存在する。それぞれの役割を以下に明記する。

| モデル名 | 定義場所 | 役割 | 使用するFEAT |
|---|---|---|---|
| `Task` | `app/domain/task/entities.py` | CTX-001の**主エンティティ**（読み書き両用）。エージェントによるタスクの作成・検索・更新操作で使用する。フィールドはRedmine Issue全体に相当し、エージェントがRedmineを操作するためのドメインオブジェクト | FEAT-001（タスク作成）, FEAT-002（タスク検索）, FEAT-003（タスク更新）, FEAT-004（優先度・スケジュール） |
| `TaskSummary` | `app/task/domain/models.py` | CTX-001の**読み取り専用ビューオブジェクト（DTO）**。ダッシュボード表示に特化した軽量モデル。urgency（緊急度）・status_label（表示用ステータス名）・redmine_url（Redmine詳細画面URL）などダッシュボード専用フィールドを持つ | FEAT-006（タスク一覧ダッシュボード）のみ |

**設計上の意図**:
- `Task` はエージェント操作のコアドメインオブジェクト。変更可能（ミュータブル相当）な操作の主体
- `TaskSummary` はビュー専用の集計・表示用 DTO。`Task` と重複するフィールド（id, title, status 等）を持つが、ダッシュボード表示に最適化したフィールド（urgency, redmine_url 等）を追加している
- FEAT-006はダッシュボードの読み取り専用機能であるため `Task` は使用せず `TaskSummary` のみを使用する。将来的にFEAT-006でタスク更新機能を追加する場合は `Task` エンティティを取り込むこと
