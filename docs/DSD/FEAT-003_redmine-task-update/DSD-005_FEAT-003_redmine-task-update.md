...
**重要**: 本設計書では以下のマッピングを採用する（Redmineのデフォルト設定とは異なる場合がある）:

| 本システムのstatus_id | 本システムの名称 | Redmine想定マッピング |
|---|---|---|
| 1 | 未着手 | Redmine status_id=1 (New) |
| 2 | 進行中 | Redmine status_id=2 (In Progress) |
| 3 | 完了 | Redmine status_id=5 (Closed) または 3 (Resolved) |
| 5 | 却下 | Redmine status_id=6 (Rejected) |

**注意**: 実際のRedmine環境のステータス設定は `GET /issue_statuses.json` で確認し、環境変数またはコンフィグファイルでマッピングを設定すること。
